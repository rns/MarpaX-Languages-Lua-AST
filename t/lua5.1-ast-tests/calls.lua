print("testing functions and calls")        -- get the opportunity to test 'type' too ;)
...
assert(type(f) == 'function')        -- testing local-function recursion
...
assert(fact ==  false )        -- testing declarations
...
a, x = unpack({1, 2}, 1, 1)
assert(a == 1 and x ==  nil )        -- testing closures
...
assert(a[1] == 1 and a[2] == 3 and a[3] == "a" and a[4] == "b")        -- testing calls with 'incorrect' arguments
...
, "extra arg")        -- test for generic load
...
assert(not a and string.find(b, "hhi"))        -- test generic load with nested functions
...
a = assert(load(read1(x)))
assert(a()(2)(3)(10) == 15)        -- test for dump/undump with upvalues
...
assert(x() == 23)
x("set")
assert(x() == 24)        -- test for bug in parameter adjustment
