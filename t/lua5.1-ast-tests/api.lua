...
print('testing C API')        -- testing allignment
...
assert(a == 2 and b == 3 and not c)        -- test that all trues are equal
...
tcheck(t, {n = 1, "alo23joao12"})        -- testing MULTRET

t = pack(T.testC("rawcall 2,-1; gettop; return .", function ( a, b)
  return 1, 2, 3, 4, a, b
end
, "alo", "joao"))
...
assert(a == 20 and b ==  false )        -- testing lua_is
...
assert(count( nil, 15) == 100)        -- testing lua_to...
...
assert(a(3) == math.deg(3) and a ~= math.deg)        -- testing errors
...
check3("cannot open xxxx", T.testC("loadfile 2; gettop; return .", "xxxx"))        -- testing table access
...
tcheck(t, {n = 1, a})        -- testing upvalues
...
T.upvalue(f, 2, "xuxu")
assert(T.upvalue(f, 2) == "xuxu")        -- testing environments
...
assert(debug.getfenv(T.upvalue) == _G)        -- userdata inherit environment

local u = T.testC"newuserdata 0; return 1"
assert(debug.getfenv(u) == a)        -- functions inherit environment
...
assert(debug.getfenv(b) == a)        -- testing locks (refs)
        -- reuse of references
...
a = T.ref({})
collectgarbage()
assert(type(T.getref(a)) == 'table')        -- colect in cl the `val' of all collected userdata
...
tt.__gc = F        -- test whether udate collection frees memory in the right time
 ...
 do 
  ...
  collectgarbage()
  assert(collectgarbage("count") <= x + 1)        -- udata with finalizer
...
n5 = T.udataval(n5)
collectgarbage()
assert(table.getn(cl) == 4)        -- check order of collection
...
a = {[x] = 1}
x = T.udataval(x)
collectgarbage()        -- old `x' cannot be collected (`a' still uses it)
...
assert(not T.testC("equal 2 3; return 1"))
assert(not T.testC("equal 2 3; return 1", 3))        -- testing lua_equal with fallbacks

 do 
  local map = {}
  local t = {__eq = function ( a, b)
    return map[a] == map[b]
  end
}
  local function f (x)
...
print'+'        -------------------------------------------------------------------------
 ...
 do         -- testing errors during GC
  ...
  for i = 1, 20, 2 do         -- mark half of them to raise error during GC
    ...
    debug.setmetatable(a[i], {__gc = function ( x)
  ...
  a = 0
  while 1 do 
    if xpcall(collectgarbage, function ( s)
      a = a + 1
    end
) then break        -- stop if no more errors
...
T.doremote(L1, "_ERRORMESSAGE = nil")        -- error: `sin' is not defined
...
assert(os.remove(t))
assert(_G.a == "aaax")        -- other generic tests
...
print'+'        -- testing some auxlib functions
