...
assert(not pcall(string.gsub, "alo", "(%0)", "a"))        -- big strings

local a = string.rep('a', 300000)
assert(string.find(a, '^a*.?$'))
assert(not string.find(a, '^a*.?b$'))
assert(string.find(a, '^a-.?$'))        -- deep nest of gsubs
...
assert(rev(rev(x)) == x)        -- gsub with tables
...
assert(string.gsub("a alo b hi", "%w%w+", t) == "a ALO b HI")        -- tests for gmatch
...
assert(a == 3)        -- tests for `%f' (`frontiers')
