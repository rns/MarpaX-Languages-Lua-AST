...
assert(tostring(a) ==  nil )        -- cannot change a protected metatable
...
assert(c ~= d and not pcall(function ( )
  return c < d
end
))
setmetatable(d, t1)
...
t2.__lt = t1.__lt
setmetatable(d, t2)
assert(c == d and c < d and not (d <= c))        -- test for several levels of calls

local i
local tt = {__call = function ( t,  ...)
  i = i + 1
  if t.f then
    return t.f( ...)
  else
