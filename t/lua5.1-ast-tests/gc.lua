...
a :test()        -- collection of functions without locals, globals, etc.
...
lim = 15
a = {}        -- fill a with `collectable' indices
...
a[1] =  nil        -- fill a with some `collectable' values (in both parts of the table)
...
a[1], a[2], a[3] = x, y, z
a[string.rep('$', 11)] = string.rep('$', 11)        -- fill a with some `collectable' values
...
x, y, z =  nil
collectgarbage()
assert(next(a) == string.rep('$', 11))        -- testing userdata
...
u, m =  nil
collectgarbage()
assert(m == 10)        -- errors during collection
...
if not rawget(_G, "_soft") then
  ...
  collectgarbage()
end        -- create many threads with self-references and open upvalues
...
while thread_id < 1000 do 
 ...
 do 
  ...
  tt.__gc = function ( o)
    assert(getmetatable(o) == tt)        -- create new objects during GC
