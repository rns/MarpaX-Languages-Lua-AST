 do         --[
  ...
  local files = {["A.lua"] = "", ["B.lua"] = "assert(...=='B');require 'A'", ["A.lc"] = "", ["A"] = "", ["L"] = "", ["XXxX"] = "", ["C.lua"] = "package.loaded[...] = 25; require'C'"}
  AA =  nil
  local extras = [[
NAME = '%s'
REQUIRED = ...
return AA]]
  ...
  AA = "x"
  try("X", "XXxX", AA)
  removefiles(files)        -- testing require of sub-packages
  ...
  package.path = oldpath        -- check 'require' error message
  ...
  assert(not f and type(err) == "string" and (when == "open" or when == "absent"))        -- testing preload
